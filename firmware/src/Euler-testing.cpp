...
// // Time between sensor readings in milliseconds.
// // For actual motion tracking, you may want 20-50 ms.
// #define SAMPLE_PERIOD_MS 15

// // Time between printed readings in milliseconds.
// // 500 ms = 2 readings per second.
// // Set this to a slower rate than SAMPLE_PERIOD_MS to avoid serial monitor flooding.
...
// // Tracks when to print next (for controlling print rate independently of sample rate)
...
// //   becomes the reference position. Subsequent readings are relative to this.
// //
// // revert
// //   If the user types "revert", readings revert to absolute orientations
// //   and are no longer relative to the zero reference.
...
//     }
//   }
// //   // If the user typed "revert", go back to absolute orientations.
//   else if (line == "revert") {
//     zeroed = false;
...
//   // Track whether it's time to print based on PRINT_PERIOD_MS
//   unsigned long currentTime = millis();
//   bool shouldPrint = (currentTime - lastPrintTime >= PRINT_PERIOD_MS);
//   if (shouldPrint) {
...
//       // Print this IMU's Euler angles (only if it's time to print).
//       if (shouldPrint) {
//         Serial.print("IMU ");
//         Serial.print(i);
//         Serial.print(" | X: ");
...
//   // This section compares neighboring IMUs (only print if it's time to print).
//   //
//   // For 2 IMUs:
