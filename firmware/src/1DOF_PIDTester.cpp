...
// // ======================================================
// // Variables for editing: OSC_...(for osc control tuning) STALL_...(for stall zeroing tuning)
// // ----------------------
...
// // Oscillation stall-based elbow zero correction.
// // During Motor 2 oscillation, the controller watches continuous reverse/unwind
// // motion. If the elbow angle fails to decrease by STALL_MOVE_DEG within
// // STALL_TIME_MS, the current position is treated as full extension and the
// // elbow-only zero offset is corrected.
// //
// // IMPORTANT: A mechanical jam can look like full extension. Test this feature
// // unloaded and confirm the reverse direction before use with a person.
...
// const float OSC_HZ = 0.1;  //0.05 default
...
// //Stall Settings
// const bool STALL_ZERO = true;
// const unsigned long STALL_TIME_MS = (1/OSC_HZ)* 150;
// const float STALL_MOVE_DEG = 5.0;
// const int STALL_MIN_PWM = 20;
...
// // Runtime state for oscillation stall detection.
// bool stallWatch = false;
// unsigned long stallStartMs = 0;
// float stallStartDeg = 0.0;
...
// void resetOscillationStallWatch();
// void updateOscillationStallZero();
...
// // Clears the current oscillation stall observation window.
// void resetOscillationStallWatch() {
//   stallWatch = false;
//   stallStartMs = 0;
//   stallStartDeg = jntAngDeg;
// }

// // Detects a likely full-extension condition during Motor 2 oscillation.
// // The timer runs only while Motor 2 is actively commanded in the logical
// // reverse/unwind direction at or above STALL_MIN_PWM.
// void updateOscillationStallZero() {
//   if (!STALL_ZERO ||
//       !imusReady ||
//       !motor2.oscOn ||
//       motor2.manOn) {
//     resetOscillationStallWatch();
//     return;
//   }

//   // pwmCmd includes motSign. Multiplying by motSign converts it back to the
//   // controller's logical direction, where a negative value means REVERSE.
//   // Zero position will not be reset if motor is commanded foward

//   float logicalPwm =
//       motor2.pwmCmd * motor2.motSign;

//   bool unwinding =
//       logicalPwm <= 0.0 &&
//       abs(motor2.pwmCommand) >= STALL_MIN_PWM;
...
//   if (now - stallStartMs < STALL_TIME_MS) {
//     return;
//   }

//   // During reverse/unwind motion the elbow angle should change.
//   float angleChange =
//       abs(stallStartDeg - jntAngDeg);

//   if (angleChange < STALL_MOVE_DEG) {
//     Serial.print("OSC_STALL_ZERO,");
//     Serial.print(now);
//     Serial.print(",start=");
//     Serial.print(stallStartDeg, 4);
//     Serial.print(",current=");
//     Serial.print(jntAngDeg, 4);
//     Serial.print(",decrease=");
//     Serial.println(angleChange, 4);
...
//   // Adequate progress was made. Start a new observation window so continuous
//   // reverse motion must keep making at least STALL_MOVE_DEG of progress during
//   // every STALL_TIME_MS interval.
//   stallStartMs = now;
//   stallStartDeg = jntAngDeg;
// }
...
//   // Run this after Motor 2 control so the detector uses the command that was
//   // actually produced during the current cycle.
//   updateOscillationStallZero();
